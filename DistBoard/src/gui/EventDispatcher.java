 ...
 * Project: com.walidmoustafa.board.WhiteBoard
    ...
    private void dispatchEvent(BoardEvent event) {
        switch (event.eventType) {
            ...
            case "loadBoard":
                ...
                break;
            case "keyTyped"://Using keyEvents to determine which shape
                Text.draw(panel.getGraphics(), event.startPoint, event.textInput);
                break;
            case "mouseMoved"://Using mouseEvents to determine which shape
                panel.addShape(new Text(event.startPoint, event.textInput));
                break;
            case "mouseDragged"://Using mouseEvents to determine which shape
                if (event.erasing) {
                ...
                panel.mainFrame.repaint();
                break;
            case "mouseReleased"://Using mouseEvents to determine which shape
                if (event.erasing) {
        ...
        while (true) {
            ...
            try {
                ...
                for (BoardEvent event : boardEvents) {
                    if (event.eventType.equals("joinRequest") && WhiteBoard.isAdmin) {
                        dispatchEvent(event);
                    }
                }
